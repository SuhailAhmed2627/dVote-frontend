...
export class MerkleTree {
	...
	constructor(depth: number) {
		for (let i = 0; i < depth; i++) this.tree.push({});
		this.depth = depth;
		this._size = 0;
	}
	...
	addElement(x: string) {
		...
		for (let d = this.depth - 1; d > 0; d--) {
			if (index % 2 == 0) this.tree[d][index + 1] = this.tree[d][index];
			index >>= 1;
	...
	proof(i: number): string[][] {
		...
		for (let i = 0; i < this.depth - 1; i++) merkleProof.push([""]);
	...
	verifyProof(leaf: string, merkleProof: string[][], merkleRoot: string) {
		const l = merkleProof.length;
		assert(leaf == merkleProof[l - 1][0] || leaf == merkleProof[l - 1][1]);
		for (let i = l - 1; i >= 0; i--) {
			const hsh = postreidon(merkleProof[i]);
			if (i > 0)
				assert(
					hsh == merkleProof[i - 1][0] || hsh == merkleProof[i - 1][1]
				);
